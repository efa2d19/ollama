...
export VERSION=${VERSION:-$(git describe --tags --first-parent --abbrev=7 --long --dirty --always)}
...
_sign_darwin() {
    ...
    if [ -n "$APPLE_IDENTITY" ]; then
        for F in dist/darwin/ollama; do
...
_build_macapp() {
    ...
    lipo -create -output dist/Ollama.app/Contents/MacOS/Ollama dist/darwin-app-arm64
    rm -f dist/darwin-app-arm64
